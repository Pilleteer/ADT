...
int Fical(int i) { return (i + (i * i)) / 2; }
void insert(hash_t *h, char *str) {
  ...
  if (h->table[pos] == NULL) {
    ...
    int i = 1;
    while (h->table[(pos + Fical(i)) % h->size] != NULL) {
      i += 1;
    }
    h->table[(pos + Fical(i)) % h->size] = newit;
  }
}
...
int search(hash_t *h, char *str) {
  ...
  int i = 0;
  while (h->table[(pos + Fical(i)) % h->size] != NULL) {
    char *toolong = h->table[(pos + Fical(i)) % h->size]->text;
    if (comparestr(toolong, str) == 0) {
      return (pos + Fical(i)) % h->size;
    }
    i = i + 1;
  }
  return -1;
